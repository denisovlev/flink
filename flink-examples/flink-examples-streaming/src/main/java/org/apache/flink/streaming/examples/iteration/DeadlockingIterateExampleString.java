 ...
 * </p>
 */
public class DeadlockingIterateExampleString {
	...
	public static void main(String[] args) throws Exception {
		...
		if (params.has("input")) {
			inputStream = env.readTextFile(params.get("input")).map(new StringInputMap());
		} else {
			System.out.println("Executing Iterate example with default input data set.");
			System.out.println("Use --input to specify file input.");
			inputStream = env.addSource(new StringSource());
		}

		// create an iterative data stream from the input with 5 second timeout
		IterativeStream<String> it = inputStream.map(new InputMap()).iterate();

		// apply the step function to get the next Fibonacci number
		// increment the counter and split the output with the output selector
		SplitStream<String> step = it.map(new Step()).split(new MySelector());
		...
		DataStream<String> numbers = step.select("output").map(new OutputMap());
	...
	private static class StringSource implements SourceFunction<String> {
		...
		public void run(SourceContext<String> ctx) throws Exception {
			...
			while (isRunning) {
				...
				ctx.collect(String.valueOf(number++));
//				Thread.sleep(1L);
			}
		}
	...
	private static class StringInputMap implements MapFunction<String, String> {
		private static final long serialVersionUID = 1L;
...
//			System.out.println("Input " + value);
	...
	public static class Step implements MapFunction<String, String> {
		private static final long serialVersionUID = 1L;
...
//			System.out.println("Step: " + value + " to " + value);
	 ...
	 */
	public static class MySelector implements OutputSelector<String> {
		private static final long serialVersionUID = 1L;
...
//				System.out.println("Selector " + value + " sending to output");
	 ...
	 */
	public static class OutputMap implements MapFunction<String, String> {
		private static final long serialVersionUID = 1L;
